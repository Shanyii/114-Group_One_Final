...
function startAgentAnalysis() {
    ...
    // 取得當前要分析的資料集（如果是自訂輸入，則使用ML資料集作為基底，但日誌做微調）
    ...
    function printNextLog() {
        ...
        if (log.text.includes("[Parser")) {
            newStepIndex = 0;
        } else if (log.text.includes("[Intent") || log.text.includes("[RAG")) {
            newStepIndex = 1;
        } else if (log.text.includes("[Summary")) {
            newStepIndex = 2;
        } else if (log.text.includes("[Quiz")) {
            newStepIndex = 3;
        } else if (log.text.includes("[Memory") || log.text.includes("[Critic") || log.text.includes("[Response")) {
            newStepIndex = 4;
        }
...
function submitQuiz() {
    ...
    let correctCount = 0;
    let reviewHtml = "";
    let wrongTopics = []; // 收集答錯的題目關聯主題，用來設計複習路徑
    ...
    // 生成個人化學習路徑 (Roadmap)
    generateRoadmap(score, wrongTopics, data.title);
...
function exportReviewReport() {
    ...
    link.download = `StudyAgent_Report_${data.title.replace(/\s+/g, '_')}.txt`;
    link.click();
